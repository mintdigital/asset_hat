...
5.  Add this to your deployment script:
    ...
    compile/transpile to regular CSS/JS before running `rake
    asset_hat:minify`. (When AssetHat is ready for Rails 3.1, `rake
    asset_hat:minify` will automatically start with `rake assets:precompile`.)
...
{Open an issue.}[https://github.com/mintdigital/asset_hat/issues] Attach a
patch or fork away.
